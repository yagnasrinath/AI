...
def findPath(board_state, i, j):
    ...
    if isValidLeftMove(board_state, i, j):
        ...
        if finalstate == states["final_proper"]:
            ...
            for (i,j) in getBestPegH1(board_state_copy):
                ret = findPath(board_state_copy, i, j)
                if ret == None:
                    continue
                else:
...
if __name__ == "__main__":
    ...
    board = getInput()
    for (i,j) in getBestPegH1(board):
        ret = findPath(board, i, j)
        if ret != None:
            print(path_list)
            print("Statistics:")
